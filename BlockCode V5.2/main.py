...
import math
import sys
from block_types.composite_block import LinearBlock, HuggingFaceCompositeBlock
from block_types.data_block import RandomTensorBlock, TextFileDataBlock
from block_types.data_converter import CSVtoTensorBlock, AutoTokenizerBlock
from block_types.run_block import InferenceBlock, TrainingBlock, EvaluationBlock
from block_types.operation_blocks import AddBlock, SumBlock, MatmulBlock
from block_types.instance_method_block import InstanceSumBlock
from block_types.composite_block import CompositeBlock

from UI.block import Block
from UI.connection import Connection
from compiler.compiler import run_and_save_running_code
from UI.visualization import animate_data_flow
...
labels = set()
last_click_time = 0
DOUBLE_CLICK_DELAY = 400  # milliseconds
...
def dfs(block, visited=None):
    ...
    if block in visited:
        return
    visited.add(block)
    print("Visited:", block.label, block.composite_block.to_dict())
                ...
                if rect.collidepoint(event.pos):
                    if model == "back":
                        ...
                        if selected_model == "Linear Layer":
                            ...
                            param_info = HuggingFaceCompositeBlock.get_param_info(None)
                            ...
                            param_info = InstanceSumBlock.get_param_info(None)
                        else:
                            param_info = []
...
def create_block_from_type(x, y, block_type, name, params=None):
    ...
    if block_type == "RandomTensor":
        param_info = RandomTensorBlock.get_param_info(None)
        composite_block = RandomTensorBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    elif block_type == "txtFile":
        param_info = TextFileDataBlock.get_param_info(None)
        composite_block = TextFileDataBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    elif block_type == "CSVtoTensor":
        param_info = CSVtoTensorBlock.get_param_info(None)
        composite_block = CSVtoTensorBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    elif block_type == "AutoTokenizer":
        param_info = AutoTokenizerBlock.get_param_info(None)
        composite_block = AutoTokenizerBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
        ...
        num_inputs = run_block.get_num_input_ports()
        num_outputs = run_block.get_num_output_ports()
    elif block_type == "Linear Layer":
        param_info = LinearBlock.get_param_info(None)
        composite_block = LinearBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    elif block_type == "Add":
        param_info = AddBlock.get_param_info(None)
        composite_block = AddBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    elif block_type == "Sum":
        param_info = SumBlock.get_param_info(None)
        composite_block = SumBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    elif block_type == "Matmul":
        param_info = MatmulBlock.get_param_info(None)
        composite_block = MatmulBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    elif block_type == "ImportHuggingFaceModel":
        param_info = HuggingFaceCompositeBlock.get_param_info(None)
        composite_block = HuggingFaceCompositeBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
        ...
        param_info = InstanceSumBlock.get_param_info(None)
        composite_block = InstanceSumBlock(name, params)
        num_inputs = composite_block.get_num_input_ports()
        num_outputs = composite_block.get_num_output_ports()
    else:
    ...
    blocks.append(Block(x, y, name, composite_block, run_block, num_inputs, num_outputs))
...
def handle_events():
    ...
    global last_click_time  # Needed for double-click tracking
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if run_button_rect.collidepoint(pos):
                print("[DEBUG] Run Code button clicked")
                run_and_save_running_code(blocks, connections)
                return  # Don't process any more click logic

            # ✅ Handle left-click on context menu
            if showing_context_menu and event.button == 1:
                menu_rect = pygame.Rect(context_menu_pos[0], context_menu_pos[1], 200, 30)
                if menu_rect.collidepoint(pos):
                    print(f"[DEBUG] Abstracting: {[b.label for b in selected_blocks]}")
                    abstract_selected_blocks()
                    showing_context_menu = False
                    return
                else:
                    showing_context_menu = False  # Clicked outside menu
            ...
            if showing_model_ui:
                ...
                # ✅ Detect double-click on a block
                for block in reversed(blocks):
                    if block.rect.collidepoint(pos):
                        current_time = pygame.time.get_ticks()
                        double_clicked = current_time - last_click_time < DOUBLE_CLICK_DELAY
                        last_click_time = current_time

                        if double_clicked and isinstance(block.composite_block, CompositeBlock):
                            expand_composite_block(block)
                            return

                        # ✅ Handle selection (multi-select with Command/Ctrl)
                    ...
                    if not (mods & (pygame.KMOD_META | pygame.KMOD_CTRL)):
                        selected_blocks.clear()

                # ✅ Check for port dragging
...
def abstract_selected_blocks():
    ...
    composite_name = f"composite_{blocks[0].__hash__()}"
    ...
    connection_info = []
    for block in selected_blocks:
        connection_info.extend(block.get_connection_metadata())


    # Find the start and end blocks
    start_composite_blocks = [b.composite_block for b in selected_blocks if not b.inputs]
    end_composite_blocks = [b.composite_block for b in selected_blocks if not b.outputs]
    selected_composite_blocks = [b.composite_block for b in selected_blocks]
    print(f"📥 Start blocks: {[b.name for b in start_composite_blocks]}")
    print(f"📤 End blocks: {[b.name for b in end_composite_blocks]}")

    composite = CompositeBlock(
        name=composite_name,
        sub_blocks=selected_composite_blocks,  # ✅ Store block objects
        start_blocks=start_composite_blocks,
        end_blocks=end_composite_blocks,
        sub_connection_info=connection_info  # ✅ Store lightweight edge metadata
    )
    ...
    ## Number of inputs per start block all added together
    num_inputs = sum(composite_block.get_num_input_ports() for composite_block in start_composite_blocks)
    num_outputs = sum(composite_block.get_num_output_ports() for composite_block in end_composite_blocks)
...
def expand_composite_block(block_to_expand):
    print(f"\n🔓 Expanding composite block: {block_to_expand.label}")

    composite = block_to_expand.composite_block
    if not isinstance(composite, CompositeBlock):
        print("❌ Block is not a composite!")
        return

    sub_blocks = composite.sub_blocks
    if not sub_blocks:
        print("⚠️ Composite block has no stored sub-blocks to expand.")
        return

    # ✅ Step 1: Add sub-blocks back to global list
    for block in sub_blocks:
        blocks.append(block)
        print(f"  - Restored: {block.label}")

    # ✅ Step 2: Build label → block mapping
    label_to_block = {block.label: block for block in sub_blocks}

    # ✅ Step 3: Recreate connections from stored metadata
    for from_label, from_idx, to_label, to_idx in composite.sub_connection_info:
        from_block = label_to_block.get(from_label)
        to_block = label_to_block.get(to_label)
        if from_block and to_block:
            from_port = from_block.output_ports[from_idx]
            to_port = to_block.input_ports[to_idx]
            connections.append(Connection(from_port, to_port))
            print(f"  - Reconnected: {from_label}[{from_idx}] → {to_label}[{to_idx}]")
        else:
            print(f"⚠️ Could not resolve: {from_label} or {to_label}")

    # ✅ Step 4: Remove the composite block and its external connections
    if block_to_expand in blocks:
        blocks.remove(block_to_expand)
    connections[:] = [conn for conn in connections
                      if conn.from_port.block != block_to_expand and conn.to_port.block != block_to_expand]

    print(f"✅ Expansion complete. Removed: {block_to_expand.label}\n")
