...
class CompositeBlock:
    def __init__(self, name, params=None, sub_blocks=None, start_blocks=None, end_blocks=None, sub_connection_info=None):
        self.name = name
        self.params = params or {}
        self.sub_blocks = sub_blocks or []
        self.start_blocks = start_blocks or []
        self.end_blocks = end_blocks or []
        self.sub_connection_info = sub_connection_info or []  # ✅ new line
        self.input_composites = []
        self.output_composites = []
    ...
    def get_num_input_ports(self):
        return len(self.input_composites)

    def get_num_output_ports(self):
        return len(self.output_composites)
    ...
    def _hidden_forward_expr(self, inputs):
        return f"{self.name}({', '.join(inputs)})"

    def forward_expr(self, inputs):
        return self._hidden_forward_expr(inputs)
...
class HuggingFaceCompositeBlock(CompositeBlock):
    ...
    def _hidden_forward_expr(self, inputs: List[str]) -> str:
