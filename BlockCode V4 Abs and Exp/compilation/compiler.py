...
def generate_model_architecture(model_name, composite_blocks):
    ...
    # Write model class
    code.append("class {model_name}(nn.Module):\n")
    code.append("    def __init__(self):\n")
    code.append("        super().__init__()\n")
    code = []
    # Add model layers
    for block in composite_blocks:
        if block.composite_block.to_source_code():
            code.append(f"self.{block.label} = {block.composite_block.to_source_code()}")
    ...
    for i, block in enumerate(composite_blocks):
        ...
        if i == len(composite_blocks) - 1:
            if block.composite_block:
                print("DEBUG:", block.composite_block.forward_expr(input_vars))
                code.append(f"output = {block.composite_block.forward_expr(input_vars)}")
        else:
            if block.composite_block:
                code.append(f"{var_names[block]} = {block.composite_block.forward_expr(input_vars)}")
