...
import pygame
import sys
from block_types.model_block import LinearBlock, HuggingFaceModelBlock
...
from compilation.export_running_code import run_and_save_running_code
...
def create_block_from_type(x, y, block_type, name, params=None):
    ...
    if block_type == "RandomTensor":
        ...
        param_info = InstanceSumBlock.get_param_info(None)
        model_block = InstanceSumBlock(name, params)
        num_inputs = model_block.get_num_input_ports()
        num_outputs = model_block.get_num_output_ports()
    else:
